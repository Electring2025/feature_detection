...
# Visualization toggle — defaults OFF since this now runs as an unattended
# post-flight batch job rather than something someone watches frame-by-frame.
# Flip to True for manual offline debugging on a single run.
SHOW_DEBUG         = True

# ROS2 publishing toggle — lets the script run completely standalone
# (no rclpy, no GUI) if set False or if rclpy isn't installed.
PUBLISH_ROS2       = True

# JPEG quality used when encoding the small thumbnail crop sent alongside
# each validated result over ROS2.
THUMB_JPEG_QUALITY = 70
THUMB_MAX_DIM      = 160   # longest side of the thumbnail, in pixels
# =============================================================================
...
# ---------------------------------------------------------------------------
# ROS2 publishing — additive only. The rest of the file's detection logic
# is untouched. If rclpy is missing or PUBLISH_ROS2 is False, every method
# below becomes a safe no-op so the script still runs completely standalone
# (prints + optional debug window), exactly as before.
# ---------------------------------------------------------------------------
class DetectionPublisher:
    """
    Wraps a ROS2 node and three String/JSON publishers:
      feature_detection/status   → {"state": "RUNNING"|"DONE"|"ERROR"}
      feature_detection/progress → {"current": int, "total": int, "image": str}
      feature_detection/result   → {"ref": str, "image": str, "score": float,
                                     "x": float, "y": float, "z": float,
                                     "thumb_b64": str|None}

    No-ops safely if rclpy import fails or PUBLISH_ROS2 = False, so callers
    never need to check whether publishing is actually active.
    """
    ...
    def __init__(self, enabled: bool = True):
        ...
        try:
            ...
            print(f"[WARN] ROS2 publishing disabled (init failed): {e}", file=sys.stderr)
            self.enabled = False
...
def _make_thumb_b64(img_bgr: np.ndarray, bbox) -> str | None:
    """
    Crop the bbox region out of the full-resolution frame and JPEG-encode
    it as a small base64 string for the GUI's thumbnail. Returns None on
    any failure — a missing/bad thumbnail must never break the result
    message or the detection run itself.
    """
    try:
        x1, y1, x2, y2 = bbox
        h, w = img_bgr.shape[:2]
...
def process_images(images_dir: str, ref_images_dir: str, threshold: float, calib_yaml: str = None,
    ...
    try:
        ...
        group_detections = {}
        # Keep one representative full-resolution frame + bbox per (group_id, ref_name)
        # purely so we can crop a thumbnail for the best-scoring detection later —
        # does not affect the existing matching/voting logic at all.
        ...
        for count, (img_idx, img_path, yaml_path) in enumerate(selected_query_images, 1):
            ...
            if frame_candidates:
                ...
                if coords:
                    ...
                    # Track the highest-scoring frame seen so far for this
                    # (group, ref) pair, purely to source a thumbnail crop
                    # for the eventual validated result. Never affects voting.
        ...
        for ref in refs:
            ...
            if ref_name in ref_group_counts and ref_group_counts[ref_name]:
                ...
                # Publish this validated, post-voting result to the GUI.
                # Thumbnail is best-effort — a failed crop/encode must never
                # block the result message itself.
                thumb_b64 = None
